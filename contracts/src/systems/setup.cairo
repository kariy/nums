...
pub mod Setup {
    ...
    impl SetupImpl of ISetup<ContractState> {
        ...
        fn admin_settle(ref self: ContractState, message_id: felt252) {
            ...
            // [Effect] Mark cancelled before any external interactions.
            pending.status = PendingStatus::Cancelled;
            store.set_pending_purchase(@pending);

            // [Interaction] Cancel the unconsumed mainnet message. This MUST succeed; if it
            // reverts the entire admin_settle reverts so the operator never thinks they
            // handled a stuck purchase while the mainnet message can still be consumed.
            // The deployment wires `bridge_messaging` to a contract exposing a
            // single-arg `start_cancellation(message_hash)` API.
            assert(config.bridge_messaging.is_non_zero(), 'Bridge: messaging zero');
            let messaging = crate::interfaces::messaging::ICancellableDispatcher {
                contract_address: config.bridge_messaging,
            };
            crate::interfaces::messaging::ICancellableDispatcherTrait::start_cancellation(
