...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, continue userActivity: NSUserActivity) {
        ...
        // Handle it inside our web view in a SPA-friendly way.
        navigateWebView(to: universalLink)
    }
    ...
    func windowScene(_ windowScene: UIWindowScene,
        ...
        if let shortcutUrl = URL.init(string: shortcutItem.type) {
            navigateWebView(to: shortcutUrl)
        }
    }
...
private extension SceneDelegate {
    func navigateWebView(to url: URL) {
        guard let webView = currentAppWebView() else { return }
        let escapedUrl = resolvedWebViewURL(url).absoluteString
            .replacingOccurrences(of: "\\", with: "\\\\")
            .replacingOccurrences(of: "'", with: "\\'")
            .replacingOccurrences(of: "\n", with: "\\n")
            .replacingOccurrences(of: "\r", with: "\\r")
        webView.evaluateJavaScript("location.href = '\(escapedUrl)'")
    }
}
